Gt.controller('GtCanvas', ['GtNavigator'], 'GtWidget', function(){
	...
	this.connectToJSEvent('mousedown', function(e){
		this.emit('mousedown', {x:e.offsetX === undefined ? e.originalEvent.layerX : e.offsetX,
			y:e.offsetY === undefined ? e.originalEvent.layerY : e.offsetY}, e.which)}, true);
	...
	GtNavigator.connect('mousemove', function(position, w){
		if (!self.isVisible())
			return;
		position = self.documentToWidgetPosition(position);
		self.emit('globalmousemove', position, w);
	});

	GtNavigator.connect('mouseup', function(position, w){
		if (!self.isVisible())
			return;
		position = self.documentToWidgetPosition(position);
		self.emit('globalmouseup', position, w);
	});
