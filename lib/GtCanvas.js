Gt.controller('GtCanvas', ['GtNavigator'], 'GtWidget', function(){
	...
	$(document).on("touchstart", "[data-GtID='" + this.uniqid + "']", function(e){
		e.preventDefault();
		e = e.originalEvent;
		...
		self.emit('touchstart', {x:x, y:y}, e);
	});
	$(document).on("touchmove", "[data-GtID='" + this.uniqid + "']", function(e){
		e.preventDefault();
		e = e.originalEvent;
		// - $(window).scrollTop();
		var x = e.touches[0].pageX - self.getElement().offset().left + $(window).scrollLeft();
		...
		self.emit('touchmove', {x:x, y:y}, e);
	});
	$(document).on("touchend", "[data-GtID='" + this.uniqid + "']", function(e){
		e.preventDefault();
		e = e.originalEvent;
		self.emit('mouseup', self.lastTouchPosition, 1);
		self.emit('globalmouseup', self.lastTouchPosition, 1);
		self.emit('touchend', self.lastTouchPosition, e);
	});
