Gt.controller('GtButton', [], 'GtWidget', function(text){
	...
	this.connectToJSEvent(Gt.touchEvents ? 'touchstart' : 'click', 'click', true);
...
GtButton.prototype.setText = function(text)
{
	...
	if (!this.buttons.length)
		this.find('.gt-button-text').html(text);
	else
...
GtButton.prototype.setPanel = function(element)
{
	...
	this.connect('click', $.proxy(function(e){
		...
		if (this.findElement('.panelBox').is(':hidden'))
		{
			...
			if (this.parent && this.parent.widgetName == 'GtBar')
				this.parent.children.forEach(function(w){
					if (w.widgetName == 'GtButton')
						w.closePanel();
				});
